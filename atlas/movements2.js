...
for (let y = 0; y < 2; y++) {
  for (let x = 0; x < 8; x++) {
    for (let j = 0; j < 4; j++) {
      atlas.animations[`move2_${y}_${x}_${j}`] = [];
      for (let i = 0; i < 3; i++) {
        let frame = {
          frame: {
            x: width * (x * 3 + i),
        ...
        atlas.animations[`move2_${y}_${x}_${j}`].push(
          `move2_${y}_${x}_${j}${i}`,
        );

        atlas.frames[`move2_${y}_${x}_${j}${i}`] = frame;
      }
    }
  }
}
...
fs.writeFile(
  './public/static/assets/characters/movements2.json',
